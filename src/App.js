...
class App extends Component {
  ...
  componentWillMount() {
    fetch('https://crossorigin.me/http://ip-api.com/json').then(response => {
      ...
      fetch(`https://crossorigin.me/http://api.openweathermap.org/data/2.5/find?q=${this.state.userCity},${this.state.userCountryCode}&units=metric&APPID=47ffea77c9fa76f064142beb0dbd9654`)
      .then(response => {
        return response.json();
      }).then(data => {
        console.log(data)
  ...
  getWeather(inputLocation) {
    fetch(`https://crossorigin.me/http://api.openweathermap.org/data/2.5/find?q=${inputLocation},uk&units=metric&APPID=47ffea77c9fa76f064142beb0dbd9654`).then(response => {
      return response.json();
    }).then(data => {
      if (data.count === 0) {
        this.setState({
